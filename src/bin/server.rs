...
fn main() -> Result<(), KVStoreError> {
    ...
    let mut channel = machnet_attach().expect("Machnet channel cannot be created!");
    ...
    //     }
    //     None => {
    //         KVStoreError::MachnetError("Failed to attach machnet".to_string());
    //     }
    // } 
    ...
    while *running.lock().unwrap() {
        ...
        let size = machnet_recv(&channel, &mut vec![0u8; 1], 1, &mut flow);
        if size > 0 {
            let store_clone = Arc::clone(&store);

            thread::spawn(move || {
                handle_client(&mut channel, store_clone, flow);
            });
        }
    }
